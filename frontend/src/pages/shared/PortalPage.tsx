...
const etaForDistance = (distanceKm: number) => `${Math.max(1, Math.round((distanceKm / 25) * 60))} min ETA`;
...
const CitizenView: React.FC<{ currentUser: ReturnType<typeof useAuth>['currentUser']; ownRequests: EmergencyRequest[]; volunteers: ReturnType<typeof useRahatStore.getState>['volunteers']; submitRequest: (event: React.FormEvent<HTMLFormElement>) => void; refreshFromStorage: () => Promise<void> }> = ({ currentUser, ownRequests, volunteers, submitRequest, refreshFromStorage }) => {
  ...
  <RequestList title="My submitted requests" requests={ownRequests} empty="No requests submitted yet." volunteers={volunteers} />
...
const RequestList: React.FC<{ title: string; requests: EmergencyRequest[]; empty: string; volunteers?: ReturnType<typeof useRahatStore.getState>['volunteers'] }> = ({ title, requests, empty, volunteers = [] }) => <section className="mt-6 bg-white border border-slate-200 rounded-xl p-5"><h2 className="text-lg font-semibold text-slate-900">{title}</h2><div className="mt-4 space-y-3">{requests.length === 0 ? <p className="text-sm text-slate-500">{empty}</p> : requests.map((request) => <Mission key={request.id} request={request} volunteers={volunteers} />)}</div></section>;
const Mission: React.FC<{ request: EmergencyRequest; volunteers?: ReturnType<typeof useRahatStore.getState>['volunteers']; children?: React.ReactNode }> = ({ request, volunteers = [], children }) => { const responder = volunteers.find((volunteer) => volunteer.id === request.assignedVolunteerId); const distance = responder ? haversineKm(responder.location, request.location) : null; return <article className="flex flex-col gap-3 rounded-lg border border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-sm font-bold text-slate-900">{request.id}</span><span className={`rounded-full px-2 py-1 text-xs font-semibold ${statusClass[request.status]}`}>{request.status.replace('_', ' ')}</span><span className="text-xs font-semibold text-slate-500">{request.severity}</span></div><p className="mt-1 font-medium text-slate-800">{request.emergencyType} for {request.peopleAffected} people</p><p className="mt-1 flex items-center gap-1 text-sm text-slate-500"><MapPin className="w-3.5 h-3.5" />{request.location.address}</p><p className="mt-1 text-sm text-slate-500 line-clamp-1">{request.description}</p>{responder && distance !== null && <p className="mt-2 text-xs font-semibold text-blue-700">{responder.name} is {distance.toFixed(1)} km away · {etaForDistance(distance)}</p>}</div>{children}</article>; };
