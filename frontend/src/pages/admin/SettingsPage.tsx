...
import { CheckCircle2, Database, Save, UserRound, Wifi, WifiOff } from 'lucide-react';
...
const SettingsPage: React.FC = () => {
  ...
  const setBackendOnline = useRahatStore((s) => s.setBackendOnline);
  ...
  const reset = async () => {
    const confirmed = window.confirm('Reset all local demo data? This cannot be undone.');
    if (!confirmed) return;
    await resetDemoData();
    await logout();
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-6">
        <section className="rounded-xl border border-slate-200 bg-white p-6"><div className="flex items-center gap-3"><div className={`rounded-lg p-2 ${backendOnline ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{backendOnline ? <Wifi className="w-5 h-5" /> : <WifiOff className="w-5 h-5" />}</div><div><h2 className="font-semibold text-slate-900">Connection mode</h2><p className="text-xs text-slate-500">Controls the status shown to operators.</p></div></div><button type="button" onClick={() => setBackendOnline(!backendOnline)} className={`mt-5 w-full rounded-lg px-4 py-2.5 text-sm font-semibold ${backendOnline ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-amber-500 text-white hover:bg-amber-600'}`}>{backendOnline ? 'Connected to backend' : 'Working offline'} </button></section>
        <section className="rounded-xl border border-red-200 bg-red-50 p-6"><div className="flex items-center gap-3"><Database className="w-5 h-5 text-red-600" /><div><h2 className="font-semibold text-red-900">Demo workspace</h2><p className="text-xs text-red-700">Reset local requests, volunteers, resources, and notifications.</p></div></div><button type="button" onClick={reset} className="mt-5 rounded-lg border border-red-300 bg-white px-4 py-2.5 text-sm font-semibold text-red-700 hover:bg-red-100">Reset demo data</button></section>
      </div>
    </div>
  </div>;
};
